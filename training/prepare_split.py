#!/usr/bin/env python3
"""Create train/val/test splits from the crops manifest on a per-video basis."""
...
def write_outputs(out_dir: Path, assignments: SplitAssignments):
...
def summarise(assignments: SplitAssignments) -> Dict[str, int]:
...
def main() -> int:
    ...
    assignments = assign_splits(video_species, ratios=ratios_normalised, seed=seed)
    write_outputs(Path(args.out_dir), assignments)
    counts = summarise(assignments)
    print("Split counts:")
